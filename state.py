...
def init_state() -> None:
    ...
    if "active_step" not in st.session_state:
        # Default to the tutorial step for first-time users
...
def format_time_display(total_minutes: float) -> str:
    """Format time for display in the header.
    
    Shows seconds when under 1 minute, otherwise shows rounded minutes.
    
    Args:
        total_minutes: The total time in minutes (can be fractional).
        
    Returns:
        A formatted string like "23 sec logged" or "5 min logged".
    """
    if total_minutes < 1:
        seconds = int(total_minutes * 60)
        return f"{seconds} sec logged"
    else:
        minutes = int(round(total_minutes))
        return f"{minutes} min logged"
