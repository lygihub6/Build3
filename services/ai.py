...
def safe_ai(module_id: str, user_message: str, session: Dict[str, Any]) -> str:
    """Safely call the Gemini API with caching and simple rate limiting.

    This helper wraps ``call_gemini_for_module`` to avoid repeated calls during a single
    Streamlit session and to throttle requests so that no more than roughly ten calls
    are made per minute. Results are cached per (module_id, user_message) pair
    within ``st.session_state["ai_cache"]``. If a cached response exists, it is
    returned immediately. If requests are issued too quickly, a short message is
    returned instead of hitting the API again.
    ...
    # Basic rate limiter: allow ~10 calls per minute (6 seconds apart)
    now = time.time()
    if now - st.session_state["ai_last_call_ts"] < 6:
        return "Please wait a few seconds before sending another request to the assistant."
